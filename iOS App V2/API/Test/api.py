...
class api:
	def __init__(self, filename):
		...
		self.base_url = "https://a99yoingli.execute-api.us-east-1.amazonaws.com/%s" % (self.stage) 
		self.headers = {"x-api-key" : self.keys['dracker-key']}

	def new_user(self, user):
		endpoint = '%s/users' % (self.base_url) 
		return requests.post(endpoint, headers=self.headers, params=user)
	...
	def create_transaction(self, user1, user2):
		...
		return (params, requests.post(endpoint, headers=self.headers, params=params))
	...
	def get_user_transactions(self, uid):
		endpoint = ('%s/transaction/' % (self.base_url)) + uid
	...
	def get_user(self, phone):
		endpoint = '%s/users/user' % (self.base_url)
		params = {}
		params['phone'] = phone
		return requests.get(endpoint, headers=self.headers, params=params)
