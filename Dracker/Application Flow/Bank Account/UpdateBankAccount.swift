...
extension UpdateBankAccount : UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let account = accounts_list[indexPath.row]
        clear_selected()
        set_default_account(phone: phone, url: account.url) { (dat) in
            print(dat.value)
    ...
    fileprivate func clear_selected() {
        for row in 0..<accounts_list.count {
            ...
            if let cell_view = cell?.accessoryView {
                UIView.animate(withDuration: 0.2, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .curveEaseOut, animations: {
                    cell_view.alpha = 0.0
                    cell_view.removeFromSuperview()
                }, completion: nil)
            }
        }
    }
